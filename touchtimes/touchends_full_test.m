...
touchdir = ('/Users/annika/Documents/projects/dsx_GABAergic_neurons/imaging/dsx-GABA-female_male_touch/touchtimes_intact_fly');
...
outputdirmean=('/Users/annika/Documents/projects/dsx_GABAergic_neurons/imaging/dsx-GABA-female_male_touch/Results_mean/intact_fly');
outputdirsingles=('/Users/annika/Documents/projects/dsx_GABAergic_neurons/imaging/dsx-GABA-female_male_touch/Results_single_exp');
...
%calculations
[touchstartframes,eventsmat,eventpeaks,eventpeaks_mean,eventpeaks_SEM,mean_event,SEM_event,touchtimes,fluo,x_events,first_touch_events]= cellfun(@(data_arr) find_touch_ends(data_arr), data_array, 'UniformOutput', false);
...
fignew2=figure('Name','simulans_first_touch_decay');
%requires package boundedline
plot_first_touch_event=boundedline(x_events{1,1},sim_mean_first,sim_SEM_first,'m');
fignew=figure('Name','simulans_mean_event_decay');
%requires package boundedline
plot_sim_event=boundedline(x_events{1,1},sim_mean_event,sim_SEM_event,'m');

cd(outputdirmean);
 saveas(fignew,'simulans_mean_event_decay','epsc');
 saveas(fignew2,'simulans_first_touches_decay','epsc');
...
%plot the mean event of Simulans
fignew2=figure('Name','virgin_first_touch_decay');
%requires package boundedline
plot_first_touch_event=boundedline(x_events{2,1},vir_mean_first,vir_SEM_first,'m');
fignew=figure('Name','virgin_mean_event_decay');
%requires package boundedline
plot_vir_event=boundedline(x_events{1,1},vir_mean_event,vir_SEM_event,'m');
%mean of male
cd(outputdirmean);
 saveas(fignew,'virgin_mean_event_decay','epsc');
 saveas(fignew2,'virgin_first_touches_decay','epsc');
...
function [touchendframes,eventsmat,eventpeaks,eventpeaks_mean,eventpeaks_SEM,mean_event,SEM_event,touchtimes,fluo,x_events,first_touch_event] = find_touch_ends(data)
...
endtimes=ends.*touchtimes_shifted;%This contains zeros for all spaces in the matrix that are not starts of touch events
%disp(starttimes);
%subtract the shifted matrix from the original one (corresponds to
%substracting the previous element from each element
endtime_intervals=((numframes-1)/framerate)-endtimes;
...
%find starttimes of the touchevents
endtimes=ends_on_time.*endtimes;%This contains zeros for all spaces in the matrix that do not end on time
%disp(starttimes);
touchends=nonzeros(endtimes);%reduce matrix to contain only starts
%calculate the frame number from the time of the touchstarts
touchendframes=round(touchends*framerate)+1;
...
events=arrayfun(@(touchend) fluo(touchend:touchend+round(eventtime*framerate)), touchendframes,'UniformOutput',false);
...
eventbases1=mean(eventsmat(((round((eventtime-basetime)*framerate)+1):(round(eventtime*framerate)+1))));
