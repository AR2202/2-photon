...
pulseframe = options.pulseframe; % this is the frame in which the pulse comes, based
            ...
            for inhib = 1:numinhibs
                ...
                for p = 1:numel(directories)
                    ...
                    newstimfilenames = arrayfun(@(f) fullfile(directoryname, ...
                        stimdir, ...
                        strrep(f.name, strcat('-stabilized', roiname), '_stim')), ...
                        files, ...
                        'uni', false);
                ...
                if (~isempty(filenames))
                    ...
                    flynumbers = cellfun(@(filename)regexp(filename, ...
                        'fly\d+(\(|\_)', 'match'), filenames, 'uni', false);
                    %extract fluorescence for all files using the
                    %(external) extract_fluo
                    %function
                    ...
                    stimuli = cellfun(@(sfilename)extract_fluo(sfilename, ...
                        parallelize), ...
                        stimfilenames, 'uni', false);
                    isStimframe = cellfun(@(stim) stim > 1, ...
                        stimuli, ...
                        'uni', false);
                    pulsestartframes = cellfun(@(isStim) strfind(isStim, [0, 1]), ...
                        isStimframe, ...
                        'uni', false);
                    pulsesendframes = cellfun(@(isStim) strfind(isStim, [1, 0]), ...
                        isStimframe, ...
                        'uni', false);
                    ...
                    outputfig2 = fullfile(outputdir, (strcat(gender, ...
                        '_', neuronpart, inhibstring, roiname, ...
                        '_mean_pulse.eps')));
                    fignew2 = figure('Name', strcat(gender, '_', ...
                        neuronpart, inhibstring, roiname, '_mean_pulse'));
                    %plot the mean with a shaded area showing the SEM
                    x2 = 1:150;
                    h = boundedline(x2, (transpose(mean_pulseav_dff)), ...
                        transpose(SEM_pulseav_dff), 'k');
                    xlim([0, 150]);
                    ylim([-0.2, 0.8])

                    hpatch2 = patch([pulseframe, ...
                        pulsedur + pulseframe, pulsedur + pulseframe, ...
                        pulseframe], ...
                    ...
                    outputmatfile = fullfile(outputdir, (strcat(gender, ...
                        '_', neuronpart, protocolname, ...
                        inhibstring, roiname, '.mat')));
                    save(outputmatfile, ...
                        'pulsedff', ...
                        'n_flies', ...
                        'mean_pulseav_dff', ...
