...
startdir=pwd;
pathname='/Users/annika/Documents/projects/dsx_GABAergic_neurons/imaging/LC10-lexA_lexAop-CSChrimson_dsx-GABA_GCaMP6f/imaging_preprocessed';
%pathname has to be the path to the folder were files to be processed are
%located
foldername='2018_02_28';%the name of the imaging folder
subfoldername='ROI';%must be a folder within the imaging folder
stackdir = fullfile(pathname,foldername,subfoldername);
outputdirv=('/Users/annika/Documents/projects/dsx_GABAergic_neurons/imaging/LC10-lexA_lexAop-CSChrimson_dsx-GABA_GCaMP6f/Results');
...
filenames(:) = {''};
        ...
        fdorsalsup = strfind(filenames,'posterior_cellbodies');
        ...
        if exist('dff4p1so') ==1
         ...
         if exist('dff4p1sms') ==1
          ...
          if exist('dff4p1sl') ==1
            ...
            writetable(T4p1sl,outputfile4p1s,'Sheet','lateral','WriteVariableNames',false); 
          end
       ...
       writetable(Tmean4p2s,outputfile4p2s,'Sheet','mean','WriteVariableNames',false);
        if exist('dff4p2so') ==1
         ...
         if exist('dff4p2sms') ==1
            ...
            writetable(T4p2sms,outputfile4p2s,'Sheet','middle_sup','WriteVariableNames',false); 
         end
          if exist('dff4p2sl') ==1
            T4p2sl=table(dff4p2sl);
            writetable(T4p2sl,outputfile4p2s,'Sheet','lateral','WriteVariableNames',false); 
          end
         if exist('dff4p2slc') ==1
            T4p2slc=table(dff4p2slc);
            writetable(T4p2slc,outputfile4p2s,'Sheet','lat_cellbodies','WriteVariableNames',false); 
         end
         if exist('dff4p2sfs') ==1
            T4p2sfs=table(dff4p2sfs);
            writetable(T4p2sfs,outputfile4p2s,'Sheet','frontal_sup','WriteVariableNames',false); 
        end
         ...
         if exist('dff4p2sds') ==1
            ...
            writetable(T4p2sds,outputfile4p2s,'Sheet','dorsal_sup','WriteVariableNames',false); 
         end
         if exist('dff4p2smd') ==1
            T4p2smd=table(dff4p2smd);
            writetable(T4p2smd,outputfile4p2s,'Sheet','middle_deep','WriteVariableNames',false); 
         end
         if exist('dff4p2sc') ==1
