...
framerate = 5.74; % frame rate in Hz
...
baseline_end = 11;
startdir=pwd;
pathname='/Users/annika/Documents/projects/dsx_GABAergic_neurons/imaging/R71G01-lexA_lexAopchrimson_dsx-GABA_GCaMP6f/imaging_preprocessed';
%pathname has to be the path to the folder were files to be processed are
%located
foldername='2017_05_03';%the name of the imaging folder
subfoldername='ROI';%must be a folder within the imaging folder
stackdir = fullfile(pathname,foldername,subfoldername);
outputdirv=('/Users/annika/Documents/projects/dsx_GABAergic_neurons/imaging/R71G01-lexA_lexAopchrimson_dsx-GABA_GCaMP6f/Results');
outputfile1p5s=strcat(foldername,'_1p5s.xls');
outputimg1p5s=strcat(foldername,'_1p5s.eps');
outputfile2p5s=strcat(foldername,'_2p5s.xls');
outputimg2p5s=strcat(foldername,'_2p5s.eps');
outputfile4p5s=strcat(foldername,'_4p5s.xls');
outputimg4p5s=strcat(foldername,'_4p5s.eps');
outputfile1p20s=strcat(foldername,'_1p20s.xls');
...
outputfile4p1s=strcat(foldername,'_4p1s.xls');
outputimg4p1s=strcat(foldername,'_4p1s.eps');
...
filenames(:) = {''};
        ...
        foverview = strfind(filenames,'overview');
        fmiddlesup = strfind(filenames,'middle_sup');
        flateral = strfind(filenames,'lateral_branch');
        flatcell = strfind(filenames,'lateral_cellbodies');
        ffrontalsup = strfind(filenames,'frontal_sup');
        fdorsaldeep = strfind(filenames,'dorsal_deep');
        ffrontallat = strfind(filenames,'frontal_lateral');
        fdorsalsup = strfind(filenames,'dorsal_sup');
        fmiddledeep = strfind(filenames,'middle_deep');
        fconnect = strfind(filenames,'connection');
        ...
        if exist('dff1p5so') ==1
         ...
         if exist('dff1p5sms') ==1
            ...
            writetable(T1p5sms,outputfile1p5s,'Sheet','middlesup','WriteVariableNames',false); 
         end
          if exist('dff1p5sl') ==1
            T1p5sl=table(dff1p5sl);
            writetable(T1p5sl,outputfile1p5s,'Sheet','lateral','WriteVariableNames',false); 
          end
         if exist('dff1p5slc') ==1
            T1p5slc=table(dff1p5slc);
            writetable(T1p5slc,outputfile1p5s,'Sheet','lat_cellbodies','WriteVariableNames',false); 
         end
         if exist('dff1p5sfs') ==1
            T1p5sfs=table(dff1p5sfs);
            writetable(T1p5sfs,outputfile1p5s,'Sheet','frontal_sup','WriteVariableNames',false); 
        end
         ...
         if exist('dff1p5sds') ==1
            ...
            writetable(T1p5sds,outputfile1p5s,'Sheet','dorsal_sup','WriteVariableNames',false); 
         end
         if exist('dff1p5smd') ==1
            T1p5smd=table(dff1p5smd);
            writetable(T1p5smd,outputfile1p5s,'Sheet','middle_deep','WriteVariableNames',false); 
         end
         if exist('dff1p5sc') ==1
        ...
        saveas(fig1p5s,outputimgv,'epsc');
        ...
        T2p5s=table(dff2p5s);
         ...
         Tmean2p5s=table(mean2p5s);
          ...
          cd(outputdirv); 
        xlswrite(outputfile2p5s,dff2p5s,1);
        xlswrite(outputfile2p5s,name2p5s,2);
        xlswrite(outputfile2p5s,mean2p5s,3);
...
%             writetable(T2p5sms,outputfile2p5s,'Sheet','middle_sup','WriteVariableNames',false); 
%          end
%           if exist('dff2p5sl') ==1
%             T2p5sl=table(dff2p5sl);
%             writetable(T2p5sl,outputfile2p5s,'Sheet','lateral','WriteVariableNames',false); 
%           end
%          if exist('dff2p5slc') ==1
%             T2p5slc=table(dff2p5slc);
%             writetable(T2p5slc,outputfile2p5s,'Sheet','lat_cellbodies','WriteVariableNames',false); 
%          end
%          if exist('dff2p5sfs') ==1
%             T2p5sfs=table(dff2p5sfs);
%             writetable(T2p5sfs,outputfile2p5s,'Sheet','frontal_sup','WriteVariableNames',false); 
%         end
...
%             writetable(T2p5sds,outputfile2p5s,'Sheet','dorsal_sup','WriteVariableNames',false); 
%          end
%          if exist('dff2p5smd') ==1
%             T2p5smd=table(dff2p5smd);
%             writetable(T2p5smd,outputfile2p5s,'Sheet','middle_deep','WriteVariableNames',false); 
%          end
%          if exist('dff2p5sc') ==1
...
%         saveas(fig2p5s,outputimg2p5s,'epsc');
       ...
       if exist('dff4p5so') ==1
         ...
         if exist('dff4p5sms') ==1
            ...
            writetable(T4p5sms,outputfile4p5s,'Sheet','middle_sup','WriteVariableNames',false); 
         end
          if exist('dff4p5sl') ==1
            T4p5sl=table(dff4p5sl);
            writetable(T4p5sl,outputfile4p5s,'Sheet','lateral','WriteVariableNames',false); 
          end
         if exist('dff4p5slc') ==1
            T4p5slc=table(dff4p5slc);
            writetable(T4p5slc,outputfile4p5s,'Sheet','lat_cellbodies','WriteVariableNames',false); 
         end
         if exist('dff4p5sfs') ==1
            T4p5sfs=table(dff4p5sfs);
            writetable(T4p5sfs,outputfile4p5s,'Sheet','frontal_sup','WriteVariableNames',false); 
        end
         ...
         if exist('dff4p5sds') ==1
            ...
            writetable(T4p5sds,outputfile4p5s,'Sheet','dorsal_sup','WriteVariableNames',false); 
         end
         if exist('dff4p5smd') ==1
            T4p5smd=table(dff4p5smd);
            writetable(T4p5smd,outputfile4p5s,'Sheet','middle_deep','WriteVariableNames',false); 
         end
         if exist('dff4p5sc') ==1
        ...
        if exist('dff1p20so') ==1
         ...
         if exist('dff1p20sms') ==1
            ...
            writetable(T1p20sms,outputfile1p20s,'Sheet','middle_sup','WriteVariableNames',false); 
         end
          if exist('dff1p20sl') ==1
            T1p20sl=table(dff1p20sl);
            writetable(T1p20sl,outputfile1p20s,'Sheet','lateral','WriteVariableNames',false); 
          end
         if exist('dff1p20slc') ==1
            T1p20slc=table(dff1p20slc);
            writetable(T1p20slc,outputfile1p20s,'Sheet','lat_cellbodies','WriteVariableNames',false); 
         end
         if exist('dff1p20sfs') ==1
            T1p20sfs=table(dff1p20sfs);
            writetable(T1p20sfs,outputfile1p20s,'Sheet','frontal_sup','WriteVariableNames',false); 
        end
         ...
         if exist('dff1p20sds') ==1
            ...
            writetable(T1p20sds,outputfile1p20s,'Sheet','dorsal_sup','WriteVariableNames',false); 
         end
         if exist('dff1p20smd') ==1
            T1p20smd=table(dff1p20smd);
            writetable(T1p20smd,outputfile1p20s,'Sheet','middle_deep','WriteVariableNames',false); 
         end
        ...
        if exist('dff2p20so') ==1
         ...
         if exist('dff2p20sms') ==1
            ...
            writetable(T2p20sms,outputfile2p20s,'Sheet','middle_sup','WriteVariableNames',false); 
         end
          if exist('dff2p20sl') ==1
            T2p20sl=table(dff2p20sl);
            writetable(T2p20sl,outputfile2p20s,'Sheet','lateral','WriteVariableNames',false); 
          end
         if exist('dff2p20slc') ==1
            T2p20slc=table(dff2p20slc);
            writetable(T2p20slc,outputfile2p20s,'Sheet','lat_cellbodies','WriteVariableNames',false); 
         end
         if exist('dff2p20sfs') ==1
            T2p20sfs=table(dff2p20sfs);
            writetable(T2p20sfs,outputfile2p20s,'Sheet','frontal_sup','WriteVariableNames',false); 
        end
         ...
         if exist('dff2p20sds') ==1
            ...
            writetable(T2p20sds,outputfile2p20s,'Sheet','dorsal_sup','WriteVariableNames',false); 
         end
         if exist('dff2p20smd') ==1
            T2p20smd=table(dff2p20smd);
            writetable(T2p20smd,outputfile2p20s,'Sheet','middle_deep','WriteVariableNames',false); 
         end
       ...
       writetable(Tmean4p1s,outputfile4p1s,'Sheet','mean','WriteVariableNames',false);
        if exist('dff4p1so') ==1
         ...
         if exist('dff4p1sms') ==1
            ...
            writetable(T4p1sms,outputfile4p1s,'Sheet','middle_sup','WriteVariableNames',false); 
         end
          if exist('dff4p5sl') ==1
            T4p1sl=table(dff4p5sl);
            writetable(T4p5sl,outputfile4p5s,'Sheet','lateral','WriteVariableNames',false); 
          end
         if exist('dff4p1slc') ==1
            T4p1slc=table(dff4p1slc);
            writetable(T4p1slc,outputfile4p1s,'Sheet','lat_cellbodies','WriteVariableNames',false); 
         end
         if exist('dff4p1sfs') ==1
            T4p1sfs=table(dff4p1sfs);
            writetable(T4p1sfs,outputfile4p1s,'Sheet','frontal_sup','WriteVariableNames',false); 
        end
         ...
         if exist('dff4p1sds') ==1
            ...
            writetable(T4p1sds,outputfile4p1s,'Sheet','dorsal_sup','WriteVariableNames',false); 
         end
         if exist('dff4p1smd') ==1
            T4p1smd=table(dff4p1smd);
            writetable(T4p1smd,outputfile4p1s,'Sheet','middle_deep','WriteVariableNames',false); 
         end
         if exist('dff4p1sc') ==1
